...
- (void)configCellWithItem:(XMNAssetModel *)item {
    ...
    if (item.previewImage) {
        ...
        self.imageView.image = item.originImage;
    }else{
    ...
    if (asset.pixelWidth < ISScreen_Width) {
        self.scrollView.zoomScale = 5.0f;
    }

    [self _resizeSubviews];
    
    if (item.type == XMNAssetTypePhoto || item.type == XMNAssetTypeLivePhoto) {
...
- (void)_setupPlayButton {
    ...
    playButton.frame = CGRectMake(0, 64, self.contentView.frame.size.width, self.contentView.frame.size.height - 64 - 44);
