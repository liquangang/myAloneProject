...
@property (nonatomic, strong) NSMutableArray * allSelectMaterialMuArray;
@end
...
- (NSMutableArray *)allSelectMaterialMuArray{
    if (!_allSelectMaterialMuArray) {
        _allSelectMaterialMuArray = [NSMutableArray new];
    }
    return _allSelectMaterialMuArray;
}
...
- (void)didFinishPickingPhoto:(NSArray<XMNAssetModel *> *)assets {
    ...
    NSMutableArray *assModelArray = [NSMutableArray new];
    for (id assetModel in assets) {
        if ([assetModel isKindOfClass:[XMNAssetModel class]]) {
            XMNAssetModel *tempModel = (XMNAssetModel *)assetModel;
            [assModelArray addObject:tempModel];
        }
    }
    ...
    self.didFinishPickingPhotosBlock ? self.didFinishPickingPhotosBlock(images,assModelArray) : nil;
}
...
- (void)setupNavigationBarAppearance {
    [self.navigationBar setBarTintColor:ColorFromRGB(0x2E2E3A, 1.0)];
    ...
    [self.navigationBar setBackgroundColor:ColorFromRGB(0x2E2E3A, 1.0)];
    ...
    [[XMNPhotoManager sharedManager] getAlbumsPickingVideoEnable:imagePickerVC.pickingVideoEnable completionBlock:^(NSArray<XMNAlbumModel *> *albums) {
        ...
        for (int i = 0; i < albums.count - 1; i++) {
            ...
            if (![albumModel.name isEqualToString:@"最近删除"]) {
                [tempMuArray addObject:albumModel];
            }
        }
        
        self.albums = [NSArray arrayWithArray:[tempMuArray copy]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XMNPhotoCollectionController *photoCollectionC = [[XMNPhotoCollectionController alloc] initWithCollectionViewLayout:[XMNPhotoCollectionController photoCollectionViewLayoutWithWidth:self.view.frame.size.width]];
    photoCollectionC.collectionViewIndexPath = indexPath;
