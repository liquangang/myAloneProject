...
- (void)getOriginImageWithAsset:(id _Nonnull)asset
    ...
    if (iOS8Later) {
        PHAsset * myAsset = (PHAsset *)asset;
        PHImageRequestOptions *imageRequestOption = [[PHImageRequestOptions alloc] init];
        imageRequestOption.synchronous = YES;
        [self.cachingImageManager requestImageForAsset:asset targetSize:CGSizeMake(myAsset.pixelWidth, myAsset.pixelHeight) contentMode:PHImageContentModeDefault options:imageRequestOption resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            resultImage = result;
            completionBlock ? completionBlock(resultImage) : nil;
        }];
//        PHAsset * myAsset = (PHAsset *)asset;
//        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:CGSizeMake(320, 320) contentMode:PHImageContentModeAspectFit options:PHImageContentModeDefault resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
//            resultImage = result;
//            completionBlock ? completionBlock(resultImage) : nil;
//        }];
    } else {
...
/**
 *  根据提供的asset获取collectionView上的缩略图
 *  使用同步方法获取
 *  @param asset           具体的asset资源 PHAsset or ALAsset
 *  @param size            缩略图大小
 *  @param completionBlock 回调block
 */
- (void)getCellThumbnailWithAsset:(id _Nonnull)asset
                             size:(CGSize)size
                  completionBlock:(void(^_Nonnull)(UIImage *_Nullable image))completionBlock{
    if (iOS8Later) {
        PHImageRequestOptions *imageRequestOptions = [[PHImageRequestOptions alloc] init];
...
- (void)getAssetPathWithAsset:(id _Nonnull)asset completionBlock:(void(^ _Nonnull)(NSString *_Nullable info))completionBlock{
    if ([asset isKindOfClass:[PHAsset class]]) {
        if (iOS8Later) {
            ...
            completionBlock ? completionBlock(assetResource.originalFilename ? [NSString stringWithFormat:@"assets-library://asset/asset.%@?id=%@&ext=%@", fileType, fileId, fileType] : nil) : nil;
        }else {
            PHImageRequestOptions *imageRequestOptions = [[PHImageRequestOptions alloc] init];
            imageRequestOptions.synchronous = YES;
            [self.cachingImageManager requestImageDataForAsset:asset options:imageRequestOptions resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                NSLog(@"11");
                NSString * fileType = [info[@"PHImageFileURLKey"] componentsSeparatedByString:@"."][1];
                NSString * fileId = [info[@"PHImageFileURLKey"] componentsSeparatedByString:@"/"][9];
                completionBlock ? completionBlock(info[@"PHImageFileURLKey"] ? [NSString stringWithFormat:@"assets-library://asset/asset.%@?id=%@&ext=%@", fileType, fileId, fileType] : nil) : nil;
            }];
        }
    } else if ([asset isKindOfClass:[ALAsset class]]) {
        ALAssetRepresentation *representation = [asset defaultRepresentation];
 ...
 *  保存视频
 */
- (void)saveVideoWithLocalUrl:(NSString*)urlString Complete:(void(^)(NSString * assetUrl))completeBlock{
    ...
    [library writeVideoAtPathToSavedPhotosAlbum:[NSURL fileURLWithPath:urlString]
                                completionBlock:^(NSURL *assetURL, NSError *error) {
                                    if (error) {
                                        ...
                                        [CustomeClass showMessage:@"视频保存成功" ShowTime:3];
                                        completeBlock(assetURL.absoluteString);
                                    }
                                }];
}
