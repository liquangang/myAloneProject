 ...
 *  @param completionBlock 回调block
 */
- (void)savePhotoWithImage:(UIImage* _Nonnull)image completion:(void (^ _Nonnull)())completion;

/**
 *  保存视频
 *  @param urlString 要保存到相册的视频文件的本地地址
 */
- (void)saveVideoWithLocalUrl:(NSString  * _Nonnull)urlString Complete:(void(^_Nonnull)(NSString *_Nonnull assetUrl))completeBlock;
@end
