...
- (NSMutableArray *)personInfoDataMuArray{
    if (_personInfoDataMuArray == nil) {
        ...
        [[NSMutableArray alloc] initWithObjects:[NSMutableArray new], [NSMutableArray new], [NSMutableArray new], nil];
    }
...
- (void)loadUserInfo{
    ...
    [[SoapOperation Singleton] WS_QueryUserInfo:^(MovierDCInterfaceSvc_VDCCustomerInfo *info) {
        ...
        [CustomeClass hudHiddenWithView:weakSelf.view Tag:123456];
        RELOADSERVERDATA([weakSelf loadUserInfo];)
    }];
}
...
didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        [CustomeClass mainQueue:^{
            ...
            weakSelf.myUserInfo.szAcatar = status;
            weakSelf.myUserInfo.nStatus = @([weakSelf.myUserInfo.nStatus intValue] + 8);
            
            [[SoapOperation Singleton] WS_SetUserInfo:weakSelf.myUserInfo
...
- (UITableView *)personInfoTable{
    ...
    if (!_personInfoTable) {
        _personInfoTable = [[UITableView alloc] initWithFrame:CGRectMake(0, - 35, ISScreen_Width, ISScreen_Height)
                                                        style:UITableViewStyleGrouped];
        _personInfoTable.delegate = self;
        _personInfoTable.dataSource = self;
    }
    return _personInfoTable;
}
