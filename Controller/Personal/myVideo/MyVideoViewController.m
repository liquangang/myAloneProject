...
- (void)viewDidLoad {
    ...
    //注册订单信息上传完成的通知
    REGISTEREDNOTI(uploadOrderInfoFinish:, orderInfoUploadSuccess);
    ...
    if([keyPath isEqualToString:@"upratio"])
    {
        ...
        if (self.videoTable.numberOfSections)
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                UploadCell * cell = [weakSelf.videoTable cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:2]];
        ...
        if (f == 1)
        {
            WEAKSELF2
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf performSelector:@selector(downloadLocalData) withObject:nil afterDelay:3];
            });
        }
...
- (void)downloadLocalData{
    ...
    if ([self.userId isEqualToString:CURRENTUSERID]) {
        ...
        [self.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:2]
                       withRowAnimation:UITableViewRowAnimationNone];
        [self.auditMuArray removeAllObjects];
        [self.auditMuArray addObjectsFromArray:[verifyArray copy]];
        [self.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:3]
                       withRowAnimation:UITableViewRowAnimationNone];
    }
}
...
- (void)downloadDataWithStart:(NSInteger)startIndex{
    WEAKSELF2
    weakSelf.isShowNoVideoLabel = YES;
    
    [CustomeClass hudShowWithView:weakSelf.view Tag:9];
    //请求已经生成的影片内容并刷新数据
    
    if ([weakSelf.showType isEqualToString:@"影片"]) {
        
        //根据用户所选类型选择下载不同类型的影片
        [[SoapOperation Singleton] WS_getvideosbyuseridWithUserId:weakSelf.userId ShareType:weakSelf.shareType Offset:startIndex Count:weakSelf.videoMuArray.count > 0 ? weakSelf.videoMuArray.count : 16 Success:^(NSMutableArray *serverDataArray) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf reloadTableWithType:myVideo StartIndex:startIndex DataArray:serverDataArray];
            });
        } Fail:^(NSError *error) {
        ...
        //加载收藏的数据
        [[SoapOperation Singleton] WS_getcollectvideosWithUserId:weakSelf.userId Offset:startIndex Count:16 Success:^(NSMutableArray *serverDataArray) {
            dispatch_sync(dispatch_get_main_queue(), ^{
                [weakSelf reloadTableWithType:collectVideo StartIndex:startIndex DataArray:serverDataArray];
            });
        } Fail:^(NSError *error) {
            [CustomeClass hudHiddenWithView:weakSelf.view Tag:9];
            RELOADSERVERDATA([weakSelf downloadDataWithStart:startIndex];)
...
- (void)reloadTableWithType:(VideoType)videoType StartIndex:(NSInteger)startIndex DataArray:(NSArray *)serverDataArray{
    WEAKSELF2
    [CustomeClass hudHiddenWithView:weakSelf.view Tag:9];
    [weakSelf.videoTable footerEndRefreshing];
    
    if (startIndex == 0 && serverDataArray.count != weakSelf.videoMuArray.count) {
        [weakSelf.videoTable addFooterWithTarget:weakSelf action:@selector(footerRefresh)];
    }
    ...
    [weakSelf.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:4]
                       withRowAnimation:UITableViewRowAnimationNone];
    if (serverDataArray.count <= 0) {
        [weakSelf.videoTable removeFooter];
        [CustomeClass showMessage:[NSString stringWithFormat:@"没有更多%@了", weakSelf.showType]
                         ShowTime:1.5];
    }
    
    if (videoType == myVideo) {
        [weakSelf downloadLocalData];
    }else if (videoType == collectVideo){
        [weakSelf.uploadMuArray removeAllObjects];
        [weakSelf.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:2]
                           withRowAnimation:UITableViewRowAnimationNone];
        [weakSelf.auditMuArray removeAllObjects];
        [weakSelf.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:3]
                           withRowAnimation:UITableViewRowAnimationNone];
    }
}
...
- (void)initUI{
    ...
    NAVIGATIONBARTITLEVIEW(@"我的影片")
    
    if (self.isShowOtherUserVideo) {
        NAVIGATIONBARTITLEVIEW(@"TA的影片")
    }
    self.cellStatus = @"大";
    self.showType = @"影片";
    //默认展示所有影片
    ...
    if (![self.userId isEqualToString:CURRENTUSERID]) {
        WEAKSELF(weaKself);
        [[SoapOperation Singleton] WS_getfriendrelationWithUserId:weaKself.userId Success:^(NSNumber *successInfo) {
    ...
    //注册cell
    [self.videoTable registerNib:[UINib nibWithNibName:@"HeaderCell" bundle:nil]
          forCellReuseIdentifier:@"userCell"];
    [self.videoTable registerNib:[UINib nibWithNibName:@"ControlCell" bundle:nil]
          forCellReuseIdentifier:@"controlCell"];
    [self.videoTable registerNib:[UINib nibWithNibName:@"VideoCell" bundle:nil]
          forCellReuseIdentifier:@"VideoCell"];
    [self.videoTable registerNib:[UINib nibWithNibName:@"VideoSecondCell" bundle:nil]
          forCellReuseIdentifier:@"VideoSecondCell"];
    [self.videoTable registerNib:[UINib nibWithNibName:@"UploadCell" bundle:nil]
          forCellReuseIdentifier:@"UploadCell"];
    [self.videoTable registerNib:[UINib nibWithNibName:@"AuditCell" bundle:nil]
          forCellReuseIdentifier:@"AuditCell"];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [cell UpdateBackImage:^(UIImageView *backImage, NSMutableDictionary * userDataDic) {
            weakSelf.headBackImage = backImage;
            weakSelf.userDataDic = [[NSMutableDictionary alloc] initWithDictionary:[userDataDic copy]];
            if ([weakSelf.userId isEqualToString:CURRENTUSERID]) {
                
                [CustomeClass mainQueue:^{
                    [weakSelf updateBackHeaderImage];
                }];
            }
        }];
        return cell;
        ...
        WEAKSELF(weakSelf);
        ControlCell * cell = [ControlCell getControlCellWithTable:tableView
        ...
        if ([self.cellStatus isEqualToString:@"大"]) {
            ...
            if (self.videoMuArray.count > indexPath.row) {
                return [VideoSecondCell getVideoSecondCellWithtable:tableView
                                                          VideoInfo:self.videoMuArray[indexPath.row]];
            }
        }
    }else if (indexPath.section == 2){
...
- (void)showSelectVideoTypeVideoWithType:(NSString *)videoType{
    if ([videoType isEqualToString:@"更换cell"]) {
        ...
        //更新table
        [self.videoTable reloadSections:[[NSIndexSet alloc] initWithIndex:4]
                       withRowAnimation:UITableViewRowAnimationNone];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 4){
        ...
        [VideoDetailViewController pushVideoDetailWithVideoInfo:[self.videoMuArray objectAtIndex:indexPath.row] Nav:self.navigationController VideoInfoBlock:^(NSMutableDictionary *videoInfoDic) {
            WEAKSELF2
            [weakSelf.videoMuArray replaceObjectAtIndex:indexPath.row withObject:videoInfoDic];
...
- (void)uploadOrderInfoFinish:(NSNotification *)noti{
    [self downloadDataWithStart:0];
}
