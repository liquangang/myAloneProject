...
{
    WEAKSELF2
    [CustomeClass HUDshow:self.view Tag:12345678];
    [CustomeClass closeInteractionEvents];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!isHavePW) {
                ...
                UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"温馨提示"
                                                                 message:@"您还未设置手机登录密码，设置后就可进行手机登陆哦～"
                                                                delegate:self
                                                       cancelButtonTitle:@"直接退出"
                                                       otherButtonTitles:@"设置密码", nil];
                [alert show];
            }
            [CustomeClass HUDhidden:weakSelf.view Tag:12345678];
            [CustomeClass openInteractionEvents];
        });
    } Fail:^(NSError *error) {
        RELOADSERVERDATA([weakSelf popLogoutActionSheet];);
    }];
}
