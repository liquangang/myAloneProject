...
@interface MusicClipsTNewViewC ()<UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, MyMusicCellDelegate>
/**  导航栏titleView(暂时不用)*/
@property (nonatomic, strong) TitleNewView * lqgTitleView;
/**  未搜索时展示的table*/
...
@implementation MusicClipsTNewViewC
...
- (void)loadData{
    ...
    [[SoapOperation Singleton] WS_GetMusicList:QueryCond Start:0 Count:@(160) Success:^(MovierDCInterfaceSvc_vpVDCMusicArray *MList) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [weakSelf.dataSource1 addObjectsFromArray:[MList.item copy]];
        ...
        RELOADSERVERDATA([weakSelf loadData];)
    }];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.tableView) {
        ...
        if (indexPath.section == 0) {
            ...
            if (musicInfo.szName) {
                [cell.musicImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@thumbail.jpg", musicInfo.szUrl]] placeholderImage:DEFAULTICON options:SDWebImageRetryFailed | SDWebImageRefreshCached];
                cell.musicName.text = musicInfo.szName;
                cell.singer.text = musicInfo.szDesc;
                cell.playButton.image = [UIImage imageNamed:@"playIcon"];
            ...
            if ([musicInfo.szName isEqualToString:@"无音乐"]){
        ...
        if (indexPath == self.selectIndex) {
            ...
            if (self.isPlayMusic == YES) {
                cell.playButton.image = [UIImage imageNamed:@"pauseIcon"];
            }
        }
    ...
    //展示搜索结果的table
        ...
        musicCell.playButton.image = [UIImage imageNamed:@"playIcon"];
        [musicCell.musicImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@thumbail.jpg", musicInfo.szUrl]] placeholderImage:DEFAULTICON options:SDWebImageRetryFailed | SDWebImageRefreshCached];
        ...
        if (indexPath == self.selectSearchIndex) {
            ...
            if (self.isPlayMusic == YES) {
                musicCell.playButton.image = [UIImage imageNamed:@"pauseIcon"];
            }
        }
