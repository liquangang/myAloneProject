...
- (id)initWithSubViewControllers:(NSArray *)subViewControllers showSubView:(BOOL)showContent;
{
    self = [super init];
    if (self)
    {
        _subViewControllers = subViewControllers;
        _showContentView = showContent;
    }
    return self;
}
...
{
    self = [self initWithSubViewControllers:subControllers showSubView:true];
...
- (void)viewInit
{
    ...
    if (_showContentView == true) {
        ...
        _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, _navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, 0)];

    _mainView.delegate = self;
    _mainView.pagingEnabled = YES;
    _mainView.bounces = _mainViewBounces;
    _mainView.showsHorizontalScrollIndicator = NO;
    _mainView.contentSize = CGSizeMake(SCREEN_WIDTH * _subViewControllers.count, DOT_COORDINATE);        [self.view addSubview:_mainView];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [_delegate itemShouldShowWithindex:_currentIndex];
}
...
//    [_delegate itemShouldShowWithindex:[_delegate itemShouldShowWithindex:_currentIndex];];
}
