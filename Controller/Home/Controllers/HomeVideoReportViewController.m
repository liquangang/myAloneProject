//
//  HomeVideoReportViewController.m
//  M-Cut
//
...
@interface HomeVideoReportViewController () <UITextViewDelegate>
/**  色情低俗 */
...
/**  盗用作品 */
@property (weak, nonatomic) IBOutlet UIButton *piracyButton;
/**  其他 */
...
@implementation HomeVideoReportViewController
...
- (IBAction)confirm:(UIButton *)sender {
    if (self.selectedButton) {
        ...
        [soap WS_ReportVideo:nil ReportType:type VideoId:@(videoID) Reason:content Success:^{
            dispatch_async(dispatch_get_main_queue(), ^{
                [UIWindow showMessage:@"举报成功, 我们会尽快处理." withTime:2.0];
...
//            dispatch_async(dispatch_get_main_queue(), ^{
//                [UIWindow showMessage:@"请检查网络!" withTime:2.0];
//            });
        }];
    }
}
...
- (NSNumber *)reportType{
    ...
    if (self.selectedButton == self.vulgarityButton) {
        ...
        type = 3;
    } else if (self.selectedButton == self.piracyButton) {
        type = 4;
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    // 设置用户输入文字的个数
    NSString *currentLength = [NSString stringWithFormat:@"%zd", length];
