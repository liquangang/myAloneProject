...
{
    NSLog(@"有%ld个banner", (unsigned long)array.count);
    WTImageScroll *selfImageScroll=[[WTImageScroll alloc]initWithFrame:rect];
...
//            [self networkImageSettingWithString:array[i>(array.count-1)?0:i] AndShowImageView:btn];
            ...
            [imageView sd_setImageWithURL:[NSURL URLWithString:array[i>(array.count-1)?0:i]] placeholderImage:DEFAULTVIDEOTHUMAIL options:SDWebImageProgressiveDownload];
        }
    ...
    pageView.currentPageIndicatorTintColor=ISRedColor;
    pageView.pageIndicatorTintColor= [UIColor groupTableViewBackgroundColor];
