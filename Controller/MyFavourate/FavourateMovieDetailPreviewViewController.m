//
//  FavourateMovieDetailPreviewViewController.m
//  M-Cut
//
...
@implementation FavourateMovieDetailPreviewViewController

- (void)performSegueWithIdentifier:(NSString *)identifier sender:(id)sender{
    if ([identifier isEqualToString:@"HomeVCPushCollectionIdentifier_segue"]) {
        
    } else if([identifier isEqualToString:@"MyFavourateIndentifier_segue"]){
...
- (void)didFinishGetUMSocialDataInViewController:(UMSocialResponseEntity *)response {
    if (response.responseCode == UMSResponseCodeSuccess) {      // 分享成功
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ret = [[APPUserPrefs Singleton] APP_increaseShareNumByVideoID:videoID];
            if (ret == -1) {
                NSLog(@"转发失败----%d", ret);
...
- (IBAction)clickSave:(id)sender
{
    ...
    if (ret == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"MessageBox" message:@"" delegate:nil cancelButtonTitle:@"已取消收藏！"otherButtonTitles:nil];
        [alert show];
    }
}
